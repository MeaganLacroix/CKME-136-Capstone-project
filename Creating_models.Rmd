...
set.seed(1)
rf_model=randomForest(other_yes_per~., data= train_new_mod1, importance = TRUE)
rf_model

```
*Not sure why rmse function not working here (think it is because of NA values). Calculating manually here* This random forest model has the best rmse value compared to all other models tested. 
```{r}
rf_model_pred <- predict(object = rf_model, newdata = test_new_mod1)
date.test.yes <- test_new_mod1$other_yes_per
sqrt(mean((rf_model_pred-date.test.yes)^2, na.rm = TRUE))
...
```
Estimating the importance of the variables in the random forest model. %IncMSE indicates the increase of the Mean Squared Error when given variable is randomly permuted. The most important variables appear to be like_o, atrr_o, and subject_yes_per. Because IncNodePurity is biased in these models it will not be used to to make any decisions about importance. 
```{r}
importance(rf_model)
varImpPlot(rf_model)
```

Determining the best value of mtry for the random forest model using tune RF. It appears that mtry = 7 is the best option
```{r}
rf.mtry <- tuneRF(train_new_mod1[, -18], train_new_mod1[ , 18], ntreeTry = 500, stepFactor = 1, doBest = TRUE)
rf.mtry

```
Tuning the hyperparameters of the random forest model using a grid search *COME BACK TO SEQUENCES SET FOR THE HYPER.GRID. JUSTIFY WITH LIT SEARCH*
```{r}
mtry <- seq(5, 21, 2) #best mtry may be 7, selecting a range above and below that
nodesize <- seq(4, 10, 2) #default for regression = 5, selecting a range above and below that
sampsize <- nrow(train_new_mod1) * c(0.6, 0.7, 0.8) #default sampsize = 63.2%, selecting a range above and below that
hyper_grid <- expand.grid(mtry = mtry, nodesize = nodesize, sampsize = sampsize)
hyper_grid
```

Writing a for loop over the rows of hyper_grid to train the grid models. *COME BACK TO THIS. LOOP NOT WORKING*
```{r}
#rf.grid.rmse <- c()
#for(i in 1:nrow(hyper_grid)) {
    
 #   rf.grid.model <- randomForest(formula = other_yes_per~., data= train_new_mod1,
  #                                   mtry = hyper_grid$mtry[i],
   #                                  nodesize = hyper_grid$nodesize[i],
    #                                 sampsize = hyper_grid$sampsize[i])
    #rf.pred <- predict(object = rf.grid.model, newdata = test_new_mod1)
    #rf.grid.rmse[i] <- sqrt(mean((rf.pred-date.test.yes)^2, na.rm = TRUE))
                                  
#}
```

*PLEASE IGNORE BELOW. COMING BACK TO THIS*
